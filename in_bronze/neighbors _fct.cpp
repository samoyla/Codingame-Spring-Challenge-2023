...
vector<vector<int>> find_targets(const vector<Cell>& cells, const vector<int>& from, int target_type) {
    ...
    vector<vector<int>> sorted_targets;

    for (size_t i = 0; i < all_targets.size(); i += 2) {
        if (i + 1 < all_targets.size()) {
            if (all_targets[i].size() <= all_targets[i + 1].size()) {
                sorted_targets.push_back(all_targets[i]);
            } else {
                sorted_targets.push_back(all_targets[i + 1]);
            }
        } else {
            sorted_targets.push_back(all_targets[i]);
        }
...
while (1) {
    ...
    //for (const auto& path : crist_paths) {
        //cerr << "beacon cond : " << (beacons_placed < nb_my_ants/2) << endl;
        if (beacons_placed < nb_my_ants / 2 && nb_my_ants > 25){
            for (int i = 0; i < path.size(); ++i) {
                if(!cells[i].beacon){
                    beacons_placed++;        
                }

...
class Cell {
    public:
        ...
        Cell(int index, int cell_type, int resources, vector<int> neighbors, int my_ants, int opp_ants, bool beacon, int total_beacons)
            : index(index), cell_type(cell_type), resources(resources), neighbors(neighbors),
            my_ants(my_ants), opp_ants(opp_ants), beacon(false), total_beacons(0) {}
       // ~Cell();
};
...
int main()
{
    ...
    for (int i = 0; i < number_of_cells; i++) {
        ...
        Cell cell(i, type, initial_resources, neighbors, 0, 0, false, 0);
